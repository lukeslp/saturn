...
The public viewer's default model workflow requires both optional extras (`pip install 'saturn-dissect[web,llm]'`). Server deployment is owned by native systemd and published through Caddy. A viewer without provider credentials remains usable: model insight fails open and the deterministic analysis is still returned.
...
### 1. Native systemd unit

Install the tracked unit and enable it:

```bash
sudo install -m 644 deploy/saturn-viewer.service /etc/systemd/system/saturn-viewer.service
sudo systemctl daemon-reload
sudo systemctl enable --now saturn-viewer.service
```

Manage it only through native systemd. The restored `service_manager.py` entry is historical and is not live authority:

```bash
systemctl status saturn-viewer.service
sudo systemctl restart saturn-viewer.service
journalctl -u saturn-viewer.service
```
...
The unit reads `/etc/dreamer/secrets.env`, which supplies `OPENAI_API_KEY` for the default `openai:gpt-5.6-luna` workflow. Store the secret outside this repository; do not put it in `scripts/start.sh`, the unit, or a tracked dotenv file. A different `SATURN_DEFAULT_LLM` requires that provider's standard environment key.
...
Environment knobs: `OPENAI_API_KEY`, `SATURN_DEFAULT_LLM`, `SATURN_FINDINGS_DIR`, `SATURN_PORT`, `SATURN_HOST`, `SATURN_WORKERS`, `SATURN_THREADS`. Defaults: `openai:gpt-5.6-luna`, `/home/coolhand/saturn-findings`, `5043`, `127.0.0.1`, `1`, `8`. `OPENAI_API_KEY` has no code default and is injected from the protected service environment. Keep one worker while background job state is process-local; the eight request threads serve reads and job polling while work runs in the bounded executor.
...
```bash
systemctl is-active saturn-viewer.service       # expect active
